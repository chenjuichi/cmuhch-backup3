...
const CRGB CURRENT_STATION_COLOR = RED_COLOR;     // 定義station的led燈條顏色

// 定義時間延遲常數
const int interval_5000 = 5000;
const int interval_2000 = 2000;
const int interval_1000 = 1000;
const int interval_500 = 500;
const int interval_200 = 200;
...
//#define pins {5, 4, 0, 2, 14}   //gpio pins: D1, D2, D3, D4, D5
...
// 定義wifi熱點及主機ip的的資料
//2023-11-09-mark const char* ssid = "staff-Dev";
//2023-11-09-mark const char* password = "cmuhch1357";
//2023-11-09-mark const char* mqttServer = "10.108.249.100";
//
//const char* ssid = "ZH";
...
const int mqttPort = 1883;              //定義mqtt port編號

// 定義esp8266板子的無線網路的ip資料
//IPAddress local_IP(10,108,249,101);   //for 熱點 staff-Dev
IPAddress local_IP(192,168,43,101);     //for 熱點 AndroidAPEFA6
...
//CRGBSet<CRGB> ledSets[NUM_STRIPS];
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // 初始化string_strLed的顯示字串
  //String string_strLed = string_mqttTopic + "/" + ledStrip_layout +"/" + ledStrip_begin + "/" + ledStrip_end + ":";
...
// 取消特定led燈條的所有分段資料
void cancelSegmentsForStrip(int stripIndex) {  
  segmentsByStrip[stripIndex - 1].clear();
}
...
void initializeLeds() {
  ...
  //  FastLED.addLeds<LED_TYPE, pins[i], GRB>(leds[i], NUM_LEDS_PER_STRIP);
    // 初始化不同led燈條的亮度
  ...
  // 初始化不同 led 燈條的索引範圍
  // for (int i = 0; i < NUM_STRIPS; ++i) {    
  //   ledSets[i] = CRGBSet<CRGB>(&leds[i][0], NUM_LEDS_PER_STRIP);
  // }

  // 初始化不同 led 燈條的亮度
  // for (int i = 0; i < NUM_STRIPS; ++i) {    
  //   ledSets[i].setBrightness(max_bright);
  // }
...
/*
void addSegment(int stripIndex, int start, int end, CRGB color, unsigned long duration) {
...
void fillSegments(unsigned long currentMillis, int stripIndex, int start, int end, int g_int_num_ret_seg) {
  if (ledFlash) {    
    ...
    if (g_int_num_ret_seg >= 0 && g_int_num_ret_seg < segmentsByStrip[stripIndex].size()) {    
      ...
      //if (allZeros(stripIndex, start, end)) {          
...
// 將指定分段的LED設置為color(stripIndex: 0~4, start/end: 1~30)
void displayOn(int stripIndex, int start, int end, CRGB color) {
  // 檢查該條 LED 燈條是否有分段資料
  if (!segmentsByStrip[stripIndex].empty()) {
    // 遍歷分段資料
    for (auto& segment : segmentsByStrip[stripIndex]) {
      // 檢查分段的起始和結束位置是否在指定範圍內
      if (segment.start >= start && segment.end <= end) {
        // 更新分段的顏色
        segment.color = color;
      }
...
// 將指定分段的LED設置為熄滅(stripIndex: 0~4, start/end: 1~30)
void displayOff(int stripIndex, int start, int end) {
  // 檢查該條 LED 燈條是否有分段資料
  if (!segmentsByStrip[stripIndex].empty()) {
    // 遍歷分段資料
    for (auto& segment : segmentsByStrip[stripIndex]) {
      ...
      if (segment.start >= start && segment.end <= end) {
        // 將分段的顏色設定為黑色
        segment.color = CRGB::Black;
      }
    }
  }
}

bool allZeros(int stripIndex, int start, int end) {
  for (int i = start; i <= end; ++i) {
    CRGB TEMP_COLOR = leds[stripIndex][i-1]; 
    Serial.println(TEMP_COLOR, CRGB(CRGB::Black));     
    if (TEMP_COLOR != CRGB(CRGB::Black)) {
      return false;  // 只要找到一個不是零的就回傳 false
    }
  }
  return true;  // 如果迴圈都執行完，表示都是零，回傳 true
}
