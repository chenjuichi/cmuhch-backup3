...
export default {
  ...
  mounted() {
    this.$emit('closeLedStrip', this.my_key);
  ...
  watch: {
    my_object: {
      handler: function (newVal, oldVal) {
          if (newVal) {
            ...
            //console.log("LedStrip, watch(), tab1_segs and segments1: ", this.tab1_segs, this.tab1_segs.segments1);
    ...
    slider_data_ok(val) {
      if (val) {
        ...
        //console.log("LedStrip, segment1: ", this.my_key, seg_len);
    ...
    slider_led_lamp_ok(val) {
      if (val) {
        ...
        for (let index=0; index<5; index++) {
          ...
          if (seg_len) {
            do {
              //console.log("dot begin/end point data: ", this.segments[Object.keys(this.segments)[index]][current_segs]);
  ...
  methods: {
    ...
    toggleLed(index) {
      ...
      //let old_seg_val=parseInt(this.segment_values[index]);
      ...
      if (ch1 && ch2) {  //range slider的值不變, segment value編號不變, 或該燈條都熄滅
        ...
        //let l1 = this.segments[Object.keys(this.segments)[index]].length;
        //console.log("hello ch1 && ch2: ", this.segments[Object.keys(this.segments)[index]], l1);
        /*
        if (l2 == l1) {
          let obj= {
      ...
      let els = document.getElementsByClassName('container_tab1');
      //let current_seg=this.segments[Object.keys(this.segments)[index]].length;
